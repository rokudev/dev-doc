...
>
> Developers can use the [**roDeviceInfo.isAutoPlayEnabled()** function](doc:ifdeviceinfo) to check whether auto-play video is enabled or disabled on a device. This function returns a flag indicating the current state of the auto-play setting. Developers can use this function to ensure that the auto-play device setting is respected when customers browse content in their apps.