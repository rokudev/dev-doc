...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>You may want certain products to only be available in specific countries. In the app, you can call the [<strong>getUserRegionData</strong>](doc:channelstore#getuserregiondata) command to determine the country associated with the user's Roku account. You can then implement business logic to filter the results of the ChannelStore [<strong>getCatalog</strong> command](doc:channelstore#getcatalog) to only display products that should be available for that country. <br /><br />If you want to offer a specific in-app product in multiple countries, you must handle currency conversion. To facilitate this, create in-app products for each country and filter out the product by the country in the app using the [<strong>ifDeviceInfo.GetCountryCode()</strong>](doc:ifdeviceinfo#getcountrycode-as-string) method.</td>
