...
<td>keys[]</td>
<td>Dynamic</td>
<td>The set of keys to be received from the timedMetaData event.<br /><br />If the keys array is empty, all the timed metadata associated with the current stream is sent with the isTimedMetaData event.<br /><br />If the keys array is invalid, then do not return any keys to the BrightScript app.<br /><br />Any keys not specified with this method are deleted by the Roku OS and never returned to the BrightScript application.</td>
</tr>
</tbody>
</table>
