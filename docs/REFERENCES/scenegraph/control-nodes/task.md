...
- A new Task node object is created for each asynchronous operation. The input data needed for the operation is set in the Task node object [\<interface\>](/docs/references/scenegraph/xml-elements/interface.md) fields in the render thread, along with an observer of the output `<interface>` field data, and the Task node control field is set to RUN. After the output data is returned to the render thread, the Task node object is not used again.
