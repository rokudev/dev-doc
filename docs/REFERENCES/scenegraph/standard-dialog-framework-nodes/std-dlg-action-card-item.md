...
| :--------- | :----- | :------ | :---------------- | :----------------------------------------------------------- |
| iconStatus | bool   | false   | READ_WRITE        | Indicates whether the **StdDlgActionCardItem** node is in the checked or unchecked state when the **iconType** field is set to "checkbox" or "radiobutton". <br /><br />The icon shown for an action card is based on the value of this field. |
| iconType   | string | "none"  | READ_WRITE        | Specifies the icon used for a **StdDlgActionCardItem** node. This may be one of the following values: ${icon-type-table} |
