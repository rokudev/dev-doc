---
title: Specifications overview
excerpt: 'Index of hardware, streaming, DRM, and Roku Search feed specifications'
deprecated: false
hidden: false
metadata:
  title: 'Specifications overview | Roku Developer Docs'
  description: 'Overview of Roku device specifications covering hardware characteristics, streaming formats, DRM, Roku Search feed formatting, and Streaming Store graphics.'
  robots: index
next:
  description: ''
---
The specifications contained in this section are intended to enable apps to
provide for and properly support all appropriate Roku devices.

This section contains the following types of information:

* [Hardware specifications](doc:hardware) — Device model code names,
  model-family numbers,
  and associated hardware characteristics (e.g., CPU and graphics device,
  system clock speed, onboard RAM and flash memory, maximum video resolution for
  UI and video playback, highest level of Wi-Fi support, supported video output
  modes, power requirements, and many more);
* [Streaming specifications](doc:media) — Video and audio file
  formats and streaming protocols;
* [DRM & content protections](doc:content-protection) — Guidelines
  and recommendations for stream
  encoding and DRM;
* [Roku Search feed format](doc:search-feed) — Specifications for
  formatting a Roku Search
  content feed, which enables content from an app to be discoverable from
  Roku's universal search feature;
* [Streaming Store graphics](doc:graphics) — Format details for
  implementation of standard graphic elements of an app's UI.
